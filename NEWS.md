...
- The `col` argument has been moved from explicit function parameters into `...` across all four base graphics wrappers (`barplot_aagi()`, `boxplot_aagi()`, `hist_aagi()`, `plot_aagi()`), providing a consistent interface and simplifying internal colour handling.
