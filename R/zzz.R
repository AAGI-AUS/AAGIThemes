  ...
  function(libname, pkgname) {
    if (any(agrepl("Proxima Nova", systemfonts::system_fonts()[, "name"]))) {
      ...
      cli::cli_warn(
        ...
        properly use {.pkg AAGIThemes} properly. If you are using Ubuntu or
        Debian Linux, you can install the fonts using the 
        {.code sudo apt update && sudo apt install ttf-mscorefonts-installer} command."
      )
    }
  }
# nocov end
