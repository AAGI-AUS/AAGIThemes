...
## How to create DPIRD style graphics

At the DPIRD Systems Modelling team, we have developed an R package, {theme.aagi}, and this R cookbook to make the process of creating publication-ready graphics in our in-house style using both R's {graphics} and {ggplot2} libraries a more reproducible process, as well as making it easier for people new to R to create beautiful graphics that adhere to DPIRD style guidelines.

### On {ggplot2} plots and graphs

Most of the focus of {theme.aagi} is given to supporting {[ggplot2](https://ggplot2.tidyverse.org/R)} visualisations, but base {graphics} plots are also supported.
The focus is given to {ggplot2} as it is more verbose and efficient in creating data visualization based on The Grammar of Graphics [@wilkinson2012grammar].
...
Several themes and colour palettes already exist for {ggplot2} to make the visualisation look professional and engaging for the end users.
{theme.aagi} leverages the ability to customise the appearance {ggplot2} to create a theme that is clean, easy to use and follows DPIRD's style guidelines including fonts and colours.
...
``` {r eval = FALSE}
if (!require("remotes"))
  remotes::install_github("DPIRD_FSI/theme.aagi",
                       build_vignettes = TRUE)
)
```

For more info on {theme.aagi} check out the [package's GitHub repository](https://DPIRD-FSI/theme.aagi), but most of the details about how to use the package and its functions are detailed below. 
...
invisible(lapply(X = x, FUN = library, character.only = TRUE))
```

## Using {theme.aagi} to create plots

{theme.aagi} provides several functions to assist users in creating plots, charts and graphs with a more unified DPIRD style.
...
#### Using base R to make a bar plot

```{r bar plot_base}
barplot_dpird(islands)
```

#### Using {ggplot2} to make a bar plot
...
#### Using base R to make a boxplot

```{r boxplot-base}
boxplot_dpird(decrease ~ treatment,
              data = OrchardSprays,
              xlab = "treatment",
              ylab = "decrease")
```

#### Using {ggplot2} to make a boxplot
...
#### Using base R to make a histogram

```{r hist-base}
library("ggplot2") # for diamonds data set
hist_dpird(diamonds$carat)
```
...
Using the {gapminder} data, we can make the following figure of life expectancy over time in Malawi with the line in DPIRD's dark blue colour.

#### Using base R to make a line graph
...
ggplot(line_df, aes(x = year, y = lifeExp)) +
  geom_line(colour = dpird_cols("dark blue")) +
  theme_aagi() +
  ylab("Life expectancy") +
  xlab("Year") +
...
#### Using base R to make a scatterplot

```{r scatter-base}
# Basic scatterplot
plot_dpird(x = data$x, y = data$y)
```

#### Using {ggplot2} to make a scatterplot
...
{theme.aagi} has colour palettes that follow the DPIRD Style Guide colourways for blue, red and green.
Each of these colourways provides three colours as shown below.

```{r dpird-colours, fig.cap="DPIRD colourways, dark red (top left), mid-red (top middle), light red (top right), dark blue (middle left), mid-blue (middle), light blue (middle right), dark green (bottom right), mid-green (bottom middle) and light green (bottom right)."}
library(scales)
show_col(dpird_colours)
```
...
Here we'll again use the {gapminder} data to construct a stacked bar chart and use DPIRD's red colourway for our colours via `scale_fill_dpird()`.
...
ggplot(data = stacked_df,
  ...
  scale_fill_dpird(colourway = "dpird red") +
...
### Using {ggplot2} for faceting

When faceting with {theme.aagi}, you can select one of the three colourways for the facet titles to correspond to your DPIRD report template.
If you do not select one of the three colour options, it will randomly select one for you.
...
col_values = unname(c(
  dpird_cols("light red"),
  dpird_cols("mid blue"),
  dpird_cols("dark red"),
  dpird_cols("dark green")
))
...
ggsave(p, filename = "DPIRD.png", path = tempdir())

print(p)
```

### Adding the DPIRD logo to your figures

Add the DPIRD logo to the upper left of the previous plot as per the style guide.
Use the `add_dpird_logo()` function to add the logo automatically to a previously saved image file.
...
library(magick)
add_dpird_logo(file_in = file.path(tempdir(), "DPIRD.png"),
               file_out = file.path(tempdir(), "DPIRD_logo.png"))

image_read(file.path(tempdir(), "DPIRD_logo.png")) |> 
...
When saving figures generated with {ggplot2} that use {theme.aagi} as the theme, PDFs will not properly embed the Arial font that is used.
...
There is no need to do anything else to make it work on this platform.
